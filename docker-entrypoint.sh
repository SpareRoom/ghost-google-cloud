...
if [ ! -e "$GHOST_CONTENT/apps" ]; then
	...
	if [[ "$*" == npm*start* ]]; then
		...
		sed -r "
			...
			s!GHOST_EMAIL_REGION!$GHOST_EMAIL_REGION!g;
			s!GHOST_EMAIL_FROM!$GHOST_EMAIL_FROM!g;
			s!GHOST_EMAIL_KEY!$GHOST_EMAIL_KEY!g;
			s!GHOST_EMAIL_SECRET!$GHOST_EMAIL_SECRET!g;
